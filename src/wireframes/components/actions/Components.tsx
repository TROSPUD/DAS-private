...
export const ActionMenuButton = React.memo(
  (props: ActionProps & ButtonProps) => {
      ...
      <>
        <Tooltip mouseEnterDelay={1} title={buildTitle(shortcut, tooltip)}>
          <Button
            {...other}
            type={type}
            className={!type ? 'menu-item' : undefined}
            disabled={disabled}
            onClick={onAction}
    ...
    <>
      <Tooltip mouseEnterDelay={1} title={buildTitle(shortcut, tooltip)}>
        <Button
          ...
          disabled={disabled}
          onClick={onAction}
          icon={buildIcon(icon, actualDisplayMode)}
        >
          {(actualDisplayMode === 'Label' ||
            actualDisplayMode === 'IconLabel') && <>{label}</>}
        </Button>
...
export function buildMenuItem(action: UIAction, key: string) {
  const { disabled, label, onAction, icon } = action
...
export const ActionMenuItem = React.memo(
  (props: ActionProps & MenuItemProps) => {
    const { action, displayMode, hideWhenDisabled, ...other } = props
    const { disabled, label, onAction, icon } = action
...
function buildIcon(
  ...
  return Types.isString(icon) ? (
    <Icon component={() => <i className={icon} />} />
  ) : (
    icon
  )
}
...
function buildTitle(shortcut: string | undefined, tooltip: string) {
  ...
  return shortcut
    ? `${tooltip} (${shortcut.replace('MOD', getModKey())})`
    : tooltip
}
